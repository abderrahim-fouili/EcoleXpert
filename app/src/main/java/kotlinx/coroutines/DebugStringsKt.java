...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m144constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m144constructorimpl = Result.m144constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
        ...
        if (Result.m147exceptionOrNullimpl(m144constructorimpl) != null) {
            m144constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
